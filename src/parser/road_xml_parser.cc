...
RoadXmlParser& RoadXmlParser::ParseLinkEle() {
  ...
  if (link_ele) {
    ...
    for (const auto& link_element : link_elements) {
      const tinyxml2::XMLElement* link_ele =
      ...
      if (link_ele) {
        ...
        double link_s;
        std::string link_dir;
        XmlQueryStringAttribute(link_ele, "elementType", link_type);
        XmlQueryIntAttribute(link_ele, "elementId", link_id);
        XmlQueryStringAttribute(link_ele, "contactPoint", link_point);
        XmlQueryDoubleAttribute(link_ele, "elementS", link_s);
        XmlQueryStringAttribute(link_ele, "elementDir", link_dir);
        if ("predecessor" == link_element) {
          ...
          if (StrEquals("start", link_point)) {
            road_ptr_->link.predecessor.point =
                core::RoadLinkInfo::Point::START;
          } else if (StrEquals("end", link_point)) {
            road_ptr_->link.predecessor.point = core::RoadLinkInfo::Point::END;
          }
          if ("+" == link_dir) {
          ...
          if (StrEquals("start", link_point)) {
            road_ptr_->link.successor.point = core::RoadLinkInfo::Point::START;
          } else if (StrEquals("end", link_point)) {
            road_ptr_->link.successor.point = core::RoadLinkInfo::Point::END;
          }
          if ("+" == link_dir) {
...
RoadXmlParser& RoadXmlParser::ParsePlanViewEle() {
  ...
  while (geometry_ele) {
    ...
    if (geometry_type_ele) {
      ...
      XmlQueryDoubleAttribute(geometry_type_ele, "curvStart", spiral_curvstart);
    ...
    if (geometry_type_ele) {
      ...
      XmlQueryStringAttribute(geometry_type_ele, "pRange", param_poly3_p_range);
      ...
      if ("ARCLENGTH" == StrToUpper(param_poly3_p_range)) {
        geometry_ptr->p_range =
            core::GeometryAttributesParamPoly3::PRange::ARCLENGTH;
      } else if ("NORMALIZED" == StrToUpper(param_poly3_p_range)) {
        geometry_ptr->p_range =
